...
def hyperbolic_distance_to_origin(
    x: torch.Tensor, 
    curv: float | torch.Tensor = 1.0, 
    eps: float = 1e-7
) -> torch.Tensor:
    """
    Compute the geodesic distance from a batch of points on the hyperboloid to the origin.
    
    Args:
        x: Tensor of shape `(B, D)` giving space components of a batch of points on the hyperboloid.
        curv: Positive scalar denoting negative hyperboloid curvature.
        eps: Small float number to avoid numerical instability.
    
    Returns:
        Tensor of shape `(B,)` giving the geodesic distance from each point to the origin.
    """
    # Compute the time component of the input points
    x_time = torch.sqrt(1 / curv + torch.sum(x**2, dim=-1))
    
    # The origin in hyperbolic space has space components [0, 0, ..., 0] and time component sqrt(1 / curv)
    origin_time = torch.sqrt(1 / curv)
    
    # Lorentzian inner product between x and the origin
    lorentz_inner_product = -x_time * origin_time
    
    # Ensure numerical stability by clamping the input to arcosh
    c_xyl = -curv * lorentz_inner_product
...
def pairwise_dist(
    x: Tensor, y: Tensor, curv: float | Tensor = 1.0, eps: float = 1e-7
) -> Tensor:
    """
...
def exp_map0(x: Tensor, curv: float | Tensor = 1.0, eps: float = 1e-7) -> Tensor:
    """
...
def log_map0(x: Tensor, curv: float | Tensor = 1.0, eps: float = 1e-7) -> Tensor:
    """
...
def half_aperture(
    x: Tensor, curv: float | Tensor = 1.0, min_radius: float = 0.1, eps: float = 1e-7
) -> Tensor:
    """
...
def oxy_angle(x: Tensor, y: Tensor, curv: float | Tensor = 1.0, eps: float = 1e-7):
    """
