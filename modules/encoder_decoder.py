...
class Transformer(nn.Module):
    def __init__(self, encoder, decoder, src_embed, tgt_embed):
        ...
        self.text_alpha = nn.Parameter(torch.tensor(1024**-0.5).log())
        self.visual_alpha = nn.Parameter(torch.tensor(1024**-0.5).log())
        curv_init=20
        learn_curv=True
        self.curv = nn.Parameter(
            torch.tensor(curv_init).log(), requires_grad=learn_curv
        )
        self.curv_visual = nn.Parameter(
            torch.tensor(curv_init).log(), requires_grad=learn_curv
        )
    ...
    def hyper_proj_visual(self, x, alpha):
        x_hp = x * alpha.exp()
        with torch.autocast(x_hp.device.type, dtype=torch.float32):
            x_hp = lorentz.exp_map0(x_hp, self.curv_visual.exp())
        return x_hp
    ...
    def encode(self, src_all, src_mask):
        ...
        # src_uni = self.hyper_proj_visual(src_uni, self.visual_alpha)

        src_uni = self.src_embed(src_uni)
        
        out = self.encoder(src_uni, src_mask,x_plip)
        return out, src_uni

    def decode(self, hidden_states, src_mask, tgt, tgt_mask):
        tgt = self.tgt_embed(tgt)
        tgt = self.hyper_proj(tgt, self.text_alpha)
    ...
    def forward(self, x, mask,x_plip):
        ...
        for i in range(self.N):
            if i == 0:
                ...
                # retrieval_text = self.hyper_proj(retrieval_text, self.text_alpha)
...
class EncoderDecoder(AttModel):
    ...
    def __init__(self, args, tokenizer):
        ...
        self.down_proj = nn.Linear(512, 1024)

        self.attn = MultiHeadedAttention(self.num_heads, 1024)
    ...
    def _forward(self, fc_feats, att_feats, report_ids, embedding=None, att_masks=None):
        ...
        if embedding is not None: 
            embedding = self.down_proj(embedding)
            att_feats_uni, _ = self.attn(att_feats_uni, embedding, embedding)
        ...
        out, em = self.model.encode(att_feats, att_masks)
        out_decode = self.model.decode(out, att_masks, report_ids, report_mask)

        outputs = F.log_softmax(self.logit(out_decode), dim=-1)
    ...
    def core(self, it, fc_feats_ph, att_feats_ph, memory, state, mask):
        ...
        out = self.model.decode(memory[0], mask, ys, subsequent_mask(ys.size(1)).to(memory[0].device))
