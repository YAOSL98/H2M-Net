...
class R2DataLoader(DataLoader):
    ...
    @staticmethod
    def collate_fn(data):
        images_id, images, coords, reports_ids, reports_masks, seq_lengths= zip(*data)
        
        images = torch.tensor(images[0]).unsqueeze(0)#改
        coords = torch.tensor(coords[0]).long()

        return images_id, images, coords, torch.LongTensor(reports_ids), torch.FloatTensor(reports_masks)
