...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, rank):
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, coords, reports_ids, reports_masks) in enumerate(tqdm(self.train_dataloader, desc='Training')):
            images, reports_ids, reports_masks = images.cuda(), reports_ids.cuda(), reports_masks.cuda()
            output = self.model(images, coords, reports_ids,  mode='train')
            # loss = self.criterion(output, reports_ids, reports_masks)

            loss1 = self.criterion(output['local'], reports_ids, reports_masks)
            loss2 = self.criterion(output['region'], reports_ids, reports_masks)
            loss3 = self.criterion(output['global'], reports_ids, reports_masks)
            loss = loss1 + loss2 + loss3 + 0.05 * (output['hyper_loss']['entail_vis'] + output['hyper_loss']['pos_vis'])
    ...
    def _val_epoch(self, rank, log):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, coords, reports_ids, reports_masks) in enumerate(tqdm(self.val_dataloader,desc='Validating')):
                images, reports_ids, reports_masks = images.cuda(), reports_ids.cuda(), reports_masks.cuda()
                # print('start eval...')
                #print("repo: ",reports_ids.shape)
                output = self.model(images, coords, mode='sample')
    ...
    def _test_epoch(self, rank, log):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, coords, reports_ids, reports_masks) in enumerate(tqdm(self.test_dataloader,desc='Testing')):
                images, reports_ids, reports_masks = images.cuda(), reports_ids.cuda(), reports_masks.cuda()
                output = self.model(images, coords, mode='sample')
